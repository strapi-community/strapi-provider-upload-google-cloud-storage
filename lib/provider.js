...
const prepareUploadFile = async (file, config, basePath, GCS) => {
  ...
  return { fileAttributes, bucketFile, fullFileName, fileExist };
};
...
const init = (providerConfig) => {
  ...
  return {
    async upload(file) {
      try {
        const { fileAttributes, bucketFile, fullFileName, fileExist } = await prepareUploadFile(
          file,
          config,
          basePath,
          GCS
        );
        if (fileExist && !config.enableMediaReplace) {
